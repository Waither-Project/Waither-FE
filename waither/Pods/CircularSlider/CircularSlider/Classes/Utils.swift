...
extension String {
    ...
    func sliderAttributeString(intFont: UIFont, decimalFont: UIFont, customDecimalSeparator: String? = nil) -> NSAttributedString {
        ...
        if #available(iOS 8.2, *) {
            integer.addAttributes([NSAttributedString.Key.font: UIFont(name: "SF Pro Bold", size: 50)], range: NSMakeRange(0, integer.length))
            decimal.addAttributes([NSAttributedString.Key.font: UIFont(name: "SF Pro Medium", size: 42)], range: NSMakeRange(0, decimal.length))
        } else {
            integer.addAttributes([NSAttributedString.Key.font: UIFont(name: "SF Pro Medium", size: 42)], range: NSMakeRange(0, integer.length))
            decimal.addAttributes([NSAttributedString.Key.font: UIFont(name: "SF Pro Medium", size: 42)], range: NSMakeRange(0, decimal.length))
        }
